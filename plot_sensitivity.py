...
metrics=['co2 stored', 'total costs', 'co2_shadow']
title={'co2 stored':'underground stored CO$_2$ (Mt CO$_2$/a)', 
       'total costs':'relative anual system cost in 2050',
       'co2_shadow': 'CO$_2$ price (€/t CO$_2$)'}
for metric in metrics:
    if metric == 'co2 stored':
        balances_df = pd.read_csv('results/version-sensitivity_cc/csvs/supply_energy.csv',
                              index_col=list(range(3)), header=list(range(4)))
        #CO2 -> Mt CO2
        balances_df= -0.000001*balances_df.loc[idx[metric, ['stores'], 'co2 stored'],idx[cluster, transmission, :,'2050']].droplevel([0,1],axis=0).droplevel([0,1,3], axis=1)
        cmap=mpl.cm.summer
        fontcolor='black'
    else:
        cost_df = pd.read_csv('results/version-sensitivity_cc/csvs/metrics.csv',
    ...
    for c in costs:
        for e in potentials:
            opts='3H-T-H-B-I-solar+p3-dist1-cb51.4ex0-co2 stored+c{}-co2 stored+e{}'.format(c,e)
    ...
    plt.savefig('figures/sensitivity_cc_{}.png'.format(metric), dpi=150, bbox_inches='tight')
