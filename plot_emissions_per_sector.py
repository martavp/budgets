...
e_ETS=pd.DataFrame(columns=pd.MultiIndex.from_product([pd.Series(data=budgets, name='budget',),
                                                       pd.Series(data=['be3', 'ex0'], name='decay',),
...
for i,budget in enumerate(budgets): 
    for j,decay in enumerate(decays[budget]):
        ...
        if j==0:
            ...
            ax1.text(2005, 2000, budget_name[budget], fontsize=14)
        if j==1:
            ax1.set_yticklabels([])
        if i!=5:
            ax1.set_xticklabels([])
...
ax1.legend(fancybox=True, fontsize=14, loc=(0.5, -1), facecolor='white', 
...
for i,budget in enumerate(budgets):  
    for j,decay in enumerate(decays[budget]):
        ...
        if i==0 and j==1:
            ax1.set_title('beta decay')
ax1.legend(fancybox=True, fontsize=14, loc=(0.5, -0.5), facecolor='white', 
           frameon=True, ncol=2, labels=e_t.index)       
